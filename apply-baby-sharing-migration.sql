...
-- Aplicar esta migração no banco de dados de produção
-- Execute via: docker exec -i olivebaby-db psql -U olivebaby -d olivebaby < apply-baby-sharing-migration.sql

-- ==========================================
-- Adicionar campo babyCpfHash ao Baby (se ainda não existir)
-- ==========================================
ALTER TABLE "babies" 
ADD COLUMN IF NOT EXISTS "baby_cpf_hash" VARCHAR(64) NULL;

-- Criar índice único apenas se a coluna não tiver constraint unique ainda
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'babies_baby_cpf_hash_key'
  ) THEN
    CREATE UNIQUE INDEX "babies_baby_cpf_hash_key" 
...
SELECT 
    'baby_members' as tabela,
    COUNT(*) as registros
FROM "baby_members"
UNION ALL
SELECT 
    'baby_invites' as tabela,
    COUNT(*) as registros
FROM "baby_invites";
